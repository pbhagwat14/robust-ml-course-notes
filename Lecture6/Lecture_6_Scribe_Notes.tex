...
The \textbf{true (or population) risk minimization} problem is \textbf{expected loss} under the true distribution $\mathbb{P^*}$.
\[
...
Let $z=(x, y)$ denote a data point, where $x$ is the input and $y$ is the target label. Let $\mathcal{P}(z)$ denote the set of all probability distributions over $z$.
...
\]
is minimized. In general, this is the framework of
...
\begin{figure}[h]
    ...
    \label{fig1}
\end{figure}
In the conventional data-driven decision-making cycle given in Figure~\ref{fig1}~\cite{blanchet2024distributionally}, we observe $n$ i.i.d.\ samples generated from the unknown
...
the pipeline as shown in Figure~\ref{fig2}~\cite{blanchet2024distributionally}.
\begin{figure}[h]
    \centering
...
\begin{itemize}[left=1em]
    ...
    using parameter $\vec{\theta}$ on data realization $\vec{z}$. In a
    classification setting, $\vec{z}=(\vec{x},y)$, and loss is measured by the
    discrepancy between $\hat{y}$ predicted using $\vec{\theta}$ and y.
\end{itemize}
...
\mathbb{P}^\star$ under i.i.d.\ assumptions, but the set $\mathcal{B}$ may still
include distributions that may be far from $\mathbb{P^\star}$ (overly
...
\begin{definition}[$\phi$-divergence]
Assume $\phi :
...
\begin{enumerate}
    ...
    \item Jensen-Shannon divergence: $\phi(t) = \tfrac{1}{2} \big( t \log t -
    (t+1)\log (\tfrac{t+1}{2}) \big)$
\end{enumerate}

\subsubsection*{KL Divergence Intuition:}
\[
H(Q, P) - H(Q) 
...
\item \emph{Identity of indiscernibles}:
    ...
    \[
        D_\phi(Q,P) = \int_{\mathcal{Z}} \phi(1) dP = 0.
    \]
...
finite real number equal to $f(x)$.~\cite{folland2013real}
\section*{Proof}
